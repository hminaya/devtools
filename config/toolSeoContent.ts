...
export const TOOL_SEO_CONTENT: Partial<Record<string, ToolSeoContent>> = {
  ...
  'stacktrace-formatter': {
    overview: 'This formatter takes a raw stack trace from JavaScript (including Node.js), Python, Java, C#, Go, PHP, or Ruby, detects the language automatically, and re-renders it with aligned frames and syntax highlighting: error types in red, function names in green, file paths in blue, and line numbers in purple. An optional anonymizer strips local user directories before you paste the trace into a bug report. Everything runs in your browser — the trace is never uploaded.',
    steps: [
      'Paste the raw trace into the input panel. The language is detected automatically from its frame syntax.',
      'Choose Format to re-render the trace with aligned indentation and color-coded frames.',
      'Enable Remove Sensitive Data first if the trace contains local paths you should not share.',
      'Copy the formatted output, or click Next Example to cycle through sample traces for each language.',
    ],
    details: [
      {
  ...
  'beautify-stack-trace': {
    overview: 'Stack traces copied out of terminals, CI logs, Docker output, or crash reporters arrive mangled: wrapped lines, stripped indentation, and log prefixes on every line. This beautifier rebuilds clean one-frame-per-line structure for JavaScript, Python, Java, C#, Go, PHP, and Ruby traces, color-codes the result — error header in red, functions in green, files in blue, line numbers in purple — and can anonymize local user paths like /Users/alice or C:\\Users\\alice before you share the output. It runs locally; nothing is uploaded.',
    steps: [
      'Paste the messy trace exactly as copied from your log, terminal, or ticket.',
      'The language is auto-detected. Choose Format to rebuild clean, indented, highlighted output.',
      'Turn on Remove Sensitive Data to replace home-directory paths with placeholders.',
      'Copy the beautified trace into your bug report, pull request, or chat message.',
    ],
    details: [
      {
        title: 'Why traces from logs look broken',
        body: 'Log shippers and terminals wrap long lines, strip leading whitespace, and prepend timestamps or container IDs to every line. That destroys the visual nesting that makes traces scannable. Beautifying restores consistent indentation so you can follow the call chain at a glance.',
      },
      {
        title: 'Clean traces get faster answers',
        body: 'A trace posted to an issue tracker is read by people who were not there when it crashed. Highlighting separates the error header and your application frames from runtime noise, and removing machine-specific paths (/home/deploy, /Users/alice) keeps internal details out of public tickets.',
      },
      {
        title: 'Where the error actually is',
        body: 'In Java, JavaScript, C#, PHP, and Ruby traces, read from the top — the first frames are where the exception surfaced. In Python, read from the bottom — the traceback ends with the failing line. Framework and library frames in between are usually context, not cause.',
      },
    ],
    faqs: [
      {
        question: 'How is this different from the Stack Trace Formatter?',
        answer: 'Both pages run the same parsing and highlighting engine, so use whichever you landed on. This page emphasizes the cleanup-and-sharing workflow: paste a mangled trace, anonymize local paths, and copy readable output.',
      },
      {
        question: 'Will beautifying change what the trace means?',
        answer: 'No. Only whitespace, alignment, coloring, and the optional path anonymization change. Frame order, function names, file names, and line numbers are preserved exactly.',
      },
      {
        question: 'Does it work on .NET or Unity traces?',
        answer: 'Yes. C#-style at Namespace.Class.Method() frames are supported, including traces copied from Unity and .NET application logs.',
      },
    ],
    references: [{ label: 'MDN: Error.prototype.stack', href: 'https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error/stack' }],
  },
  'mime-type-lookup': {
