...
function SHA1Generator() {
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');

  const generateHash = () => {
    if (input.trim()) {
      const hash = generateSHA1(input);
      setOutput(hash);
    }
  };
  ...
  const generateRandomHash = () => {
    ...
    // Hash the random text to get SHA-1
    const hash = generateSHA1(randomText);
    setInput(randomText);
    setOutput(hash);
  };
  ...
  return (
    <ToolLayout
      title="SHA-1 Hash"
      description="Generate SHA-1 hashes from text (for checksums, not security)"
    >
      <div className="space-y-4">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-md p-4">
          <p className="text-amber-800 text-sm">
            <strong>Note:</strong> SHA-1 is not cryptographically secure. Use for checksums, cache busting, or development only.
          </p>
        </div>
            ...
            <div className="flex flex-col gap-2" aria-live="polite">
              ...
              {output ? (
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-md p-4 h-full flex items-center justify-center">
                  <p className="text-slate-400">SHA-1 hash will appear here</p>
                </div>
              )}
            </div>
          </div>
