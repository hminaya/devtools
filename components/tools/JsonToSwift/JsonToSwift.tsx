...
import { convertJsonToSwift } from '../../../utils/jsonToSwift';
...
function JsonToSwift() {
  ...
  useEffect(() => {
    ...
    if (jsonInput) {
      ...
      // Auto-convert
      const result = convertJsonToSwift([jsonInput], {
  ...
  const convert = () => {
    ...
    const result = convertJsonToSwift(inputs, {
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex gap-3 items-center flex-wrap">
          <Button label="Convert to Swift" onClick={convert} variant="primary" />
        ...
        {showMultipleInputs && (
          <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
            <p className="text-blue-800 text-sm">
              <strong>Tip:</strong> Provide multiple JSON samples to detect optional fields. Properties that don't appear in all samples will be marked with <code className="bg-blue-100 px-1 rounded">?</code>
            </p>
          </div>
        )}
            ...
            {output ? (
              <>
                <CodeDisplay
                  ...
                  label="Swift Structs"
                />
                <CopyButton text={output} label="Copy Swift Code" />
              </>
            ) : (
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-slate-700">Swift Structs</label>
        ...
        {example && (
          <div className="space-y-2">
            ...
            <CodeDisplay code={example} language="swift" />
          </div>
        )}
      </div>
    </ToolLayout>
