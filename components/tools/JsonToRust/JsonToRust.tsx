...
import { convertJsonToRust } from '../../../utils/jsonToRust';
...
function JsonToRust() {
  ...
  const convert = () => {
    ...
    const result = convertJsonToRust(inputs, {
    ...
    >
      <div className="space-y-4">
        <div className="flex gap-3 items-center flex-wrap">
          <Button label="Convert to Rust" onClick={convert} variant="primary" />
          ...
          <div className="flex items-center gap-2 ml-4">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm text-slate-700">Multiple JSON samples (detect Option fields)</span>
            </label>
          </div>
        </div>
        ...
        {showMultipleInputs && (
          <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
            <p className="text-blue-800 text-sm">
              <strong>Tip:</strong> Provide multiple JSON samples to detect optional fields. Fields that don't appear in all samples will be wrapped in <code className="bg-blue-100 px-1 rounded">Option&lt;T&gt;</code>.
            </p>
          </div>
        )}
            ...
            {output ? (
              <>
                <CodeDisplay
                  ...
                  label="Rust Structs"
                />
                <CopyButton text={output} label="Copy Rust" />
              </>
            ) : (
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-slate-700">Rust Structs</label>
