...
import { convertJsonToJsDoc } from '../../../utils/jsonToJsDoc';

function JsonToJsDoc() {
    ...
    useEffect(() => {
        ...
        if (jsonInput) {
            ...
            // Auto-convert
            const result = convertJsonToJsDoc([jsonInput], {
                rootTypedefName: 'ApiResponse',
            });
    ...
    const convert = () => {
        ...
        const result = convertJsonToJsDoc(inputs, {
            rootTypedefName: rootName || 'Root',
        });
    ...
    return (
        <ToolLayout
            title="JSON to JSDoc"
            description="Convert JSON objects to JSDoc @typedef definitions for JavaScript type hints"
            fullWidth
        >
            <div className="space-y-4">
                {/* Action Buttons */}
                <div className="flex gap-3 items-center flex-wrap">
                    <Button label="Convert to JSDoc" onClick={convert} variant="primary" />
                ...
                {showMultipleInputs && (
                    <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
                        <p className="text-blue-800 text-sm">
                            <strong>Tip:</strong> Provide multiple JSON samples to detect optional fields. Fields that don't appear in all samples will be wrapped in <code className="bg-blue-100 px-1 rounded">[brackets]</code> to indicate they're optional.
                        </p>
                    </div>
                )}
                        ...
                        {output ? (
                            <>
                                <CodeDisplay
                                    code={output}
                                    language="javascript"
                                    label="JSDoc Typedefs"
                                />
                                <CopyButton text={output} label="Copy JSDoc" />
                            </>
                        ) : (
                            <div className="flex flex-col gap-2">
                                <label className="text-sm font-medium text-slate-700">JSDoc Typedefs</label>
